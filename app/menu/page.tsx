...
import { Instagram, Facebook, Menu as MenuIcon, X, MapPin, Camera } from 'lucide-react';
...
type SelectedImage = { src: string; name: string; en?: string; price: string };
...
export default function MenuPage() {
  ...
  const collegePointMenu: MenuSection[] = [
    ...
    {
      title: 'Jugos Naturales y Batidos / Natural Juices & Shakes', emoji: '🥤',
    ...
    },
    {
      ...
      items: [
        ...
        { es: 'Chimi De Pollo Sandwich',                  en: 'Cabbage, Ketchup, Mayo w/Chicken Sandwich',         price: '$6.00', img: IMG('SaborRestaurantAndBakery_ChickenWarp.jpg') },
    ...
    },
    {
      ...
      items: [
        ...
        { es: 'Tres Leches',      price: '$4.00', img: IMG('SaborRestaurantAndBakery_RresLechesCake.jpg') },
  ...
  const openImage = (item: MenuItem) => {
    if (item.img) setSelectedImage({ src: item.img, name: item.es, en: item.en, price: item.price });
  };
  ...
  return (
    <div className="min-h-screen bg-black text-white overflow-x-hidden">
      ...
      {selectedImage && (
        ...
        >
          <div
            className="relative w-full max-w-lg bg-zinc-950 rounded-3xl overflow-hidden border border-amber-500/30 shadow-2xl shadow-amber-500/10"
            onClick={e => e.stopPropagation()}
          >
            {/* Close button */}
            <button
              onClick={() => setSelectedImage(null)}
              className="absolute top-4 right-4 z-10 bg-black/60 hover:bg-black/80 text-white w-10 h-10 rounded-full flex items-center justify-center transition-all"
            >
              <X size={20} />
            </button>
            ...
            <div className="p-6">
              ...
              <div className="flex items-end justify-between gap-4">
                <h3 className="text-2xl font-black text-white leading-tight">{selectedImage.name}</h3>
                <span className="text-3xl font-black text-amber-400 shrink-0">{selectedImage.price}</span>
              </div>
            </div>
          </div>
        </div>
      ...
      <nav className="fixed top-0 w-full z-50 bg-black/90 backdrop-blur-xl shadow-2xl py-2">
        <div className="max-w-7xl mx-auto px-6 lg:px-8">
          <div className="flex justify-between items-center h-20">
            <div className="text-2xl md:text-3xl font-bold"><PotIcon /></div>

            <div className="hidden md:flex space-x-8 items-center">
              {[['/', 'Home'], ['/about', 'About'], ['/menu', 'Menu'], ['/catering', 'Catering'], ['/contact', 'Contact']].map(([href, label]) => (
        ...
        <div className={`md:hidden absolute top-20 left-0 w-full bg-black/95 backdrop-blur-xl transition-all duration-300 ${isMenuOpen ? 'opacity-100 visible' : 'opacity-0 invisible'}`}>
          <div className="flex flex-col space-y-4 p-6">
            {[['/', 'Home'], ['/about', 'About'], ['/menu', 'Menu'], ['/catering', 'Catering'], ['/contact', 'Contact']].map(([href, label]) => (
              <Link key={href} href={href} className={`text-lg transition ${href === '/menu' ? 'text-amber-400' : 'hover:text-amber-400'}`} onClick={() => setIsMenuOpen(false)}>{label}</Link>
            ))}
          </div>
        </div>
      </nav>
      ...
      >
        <div className="relative z-10 text-center px-4">
          <h1 className="text-6xl md:text-7xl font-black bg-gradient-to-r from-amber-400 via-orange-500 to-red-500 bg-clip-text text-transparent mb-4">
            Our Menu
          </h1>
          <p className="text-xl text-white/80">Select your location · Tap any item with a <Camera className="inline w-5 h-5 text-amber-400" /> to see the dish</p>
        </div>
      </section>
        ...
        <div className="max-w-7xl mx-auto px-6 lg:px-8">
          <div className="flex flex-wrap gap-3 justify-center">
            {locations.map((loc) => (
              ...
              >
                <span className="flex items-center gap-2"><MapPin size={16} />{loc.name}</span>
              </button>
            ))}
          </div>
          ...
          {activeLocation === 'college-point' && (
            <div className="space-y-10">
              <div className="text-center mb-10">
                ...
                <p className="text-gray-400 text-sm">15-20 College Point Blvd, College Point, NY 11356 · Open 7 Days 6:30 AM – 10:00 PM</p>
                <p className="text-amber-500/70 text-sm mt-2 flex items-center justify-center gap-1">
                  <Camera size={14} /> Tap items with this icon to see a photo
                </p>
              </div>

              {collegePointMenu.map((section) => (
                <div key={section.title} className="bg-zinc-900/60 rounded-2xl border border-zinc-800 overflow-hidden">
                  {/* Section header */}
                  <div className="px-6 py-5 bg-zinc-900 border-b border-zinc-800 flex items-center gap-3">
                  ...
                  <div className="divide-y divide-zinc-800/40">
                    {section.items.map((item, i) => (
                      <div
                        ...
                        className={`flex items-center gap-3 px-6 py-3.5 transition-colors group ${item.img ? 'cursor-pointer hover:bg-amber-500/8 active:bg-amber-500/15' : ''}`}
                      >
                        {/* Camera indicator */}
                        <div className="w-5 shrink-0 flex justify-center">
                          {item.img && (
                            <Camera size={14} className="text-amber-500/50 group-hover:text-amber-400 transition-colors" />
                          )}
                        </div>

                        {/* Name + description */}
                        <div className="flex-1 min-w-0">
                          <p className={`font-semibold text-sm md:text-base leading-snug ${item.img ? 'group-hover:text-amber-300' : ''} transition-colors`}>
                            {item.es}
                          </p>
                          {item.en && (
                            <p className="text-gray-500 text-xs mt-0.5 leading-snug">{item.en}</p>
                          )}
                        </div>

                        {/* Price */}
                        <span className="text-amber-400 font-bold text-sm whitespace-nowrap shrink-0">{item.price}</span>
                      </div>
                    ))}
                  </div>
                </div>
          ...
          {activeLocation === 'queens' && (
            <div className="text-center py-24">
              <div className="bg-gradient-to-br from-amber-500/10 to-orange-500/10 p-16 rounded-3xl border border-amber-500/30 max-w-2xl mx-auto">
                ...
                <h2 className="text-4xl font-black mb-4 bg-gradient-to-r from-amber-400 to-orange-500 bg-clip-text text-transparent">Queens Library Menu</h2>
